...
/** Utility functions and constants for views */

@objc class ViewUtil : NSObject { // @obj to use from objc code
    ...
    static let DefaultTextColor = ViewUtil.color(red: 80, green: 80, blue: 80, alpha: 1)
    ...
    static let LightGrayColor = ViewUtil.color(argb: 0xffe5e5e5)
    ...
    /** Color in red/green/blue, similar to Android. Black would be 0x000000. */
    class func color(rgb rgb: UInt) -> UIColor {
        // inspired from http://stackoverflow.com/a/24074509/1121497

        let argb = rgb | 0xFF000000;

        return ViewUtil.color(argb: argb)
    }
    ...
    class func color(argb argb: UInt) -> UIColor {
        ...
        let alpha = Float((argb & 0xFF000000) >> 24) / 255.0
        
        guard alpha > 0 else { fatalError("alpha is 0, you probably forgot it") }
    ...
    /** Creates label using SecondFontName and DefaultTextColor */
    class func labelSecondFontWithSize(size: Float) -> CustomLabel {
        return labelSecondFontWithSize(size, color: DefaultTextColor)
    }

    /** Creates label using SecondFontName */
    class func labelSecondFontWithSize(size: Float, color: UIColor) -> CustomLabel {
        return labelWithFont(fontSecondWithSize(size), color: color)
    }
    ...
    /** Scaled image that keeps aspect */
    class func imageScaled(imageName: String) -> UIImageView
    {
