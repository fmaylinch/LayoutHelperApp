...
+ (void)configureScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView mainView:(UIView*)mainView {
    ...
    [self configureVerticalScrollView:scrollView contentView:contentView container:mainView];
}

+ (UIView*)configureVerticalScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView {
    
    return [self configureScrollView:scrollView contentView:contentView axis:UILayoutConstraintAxisVertical];
}

+ (void)configureVerticalScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView container:(UIView*)container {

    [self configureScrollView:scrollView contentView:contentView container:container axis:UILayoutConstraintAxisVertical];
}

+ (UIView*)configureHorizontalScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView {

    return [self configureScrollView:scrollView contentView:contentView axis:UILayoutConstraintAxisHorizontal];
}

+ (void)configureHorizontalScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView container:(UIView*)container {

    [self configureScrollView:scrollView contentView:contentView container:container axis:UILayoutConstraintAxisHorizontal];
}

+ (UIView*)configureScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView axis:(UILayoutConstraintAxis)axis {
    
    UIView* container = [[UIView alloc] init];
    [self configureView:container fillWithSubView:scrollView];
    [self configureScrollView:scrollView contentView:contentView container:container axis:axis];
    return container;
}

+ (void)configureScrollView:(UIScrollView*)scrollView contentView:(UIView*)contentView container:(UIView*)container axis:(UILayoutConstraintAxis)axis {

    [AutolayoutHelper configureView:scrollView fillWithSubView:contentView];

    NSString* crossAxisLetter = axis == UILayoutConstraintAxisVertical ? @"H" : @"V";

    [container addConstraints:[NSLayoutConstraint
            constraintsWithVisualFormat:[NSString stringWithFormat:@"%@:[contentView(==container)]", crossAxisLetter]
                                options:(NSLayoutFormatOptions) 0
                                metrics:nil
                                  views:VarBindings(contentView, container)]];
}
