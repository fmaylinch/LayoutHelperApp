...
public class LayoutHelper {
    ...
    public convenience init(controller: UIViewController) {
        ...
        addGuides([
                LayoutHelper.TopGuideKey:controller.topLayoutGuide,
                LayoutHelper.BottomGuideKey:controller.bottomLayoutGuide])
    }
    ...
    public func fillWithView(view: UIView, margins:UIEdgeInsets) -> LayoutHelper
    {
        ...
        let topKey = viewOfController ? "[\(LayoutHelper.TopGuideKey)]" : "|"
        let bottomKey = viewOfController ? "[\(LayoutHelper.BottomGuideKey)]" : "|"

        let key = LayoutHelper.defaultViewKey
     ...
     */
    public func configureScrollView(scrollViewKey: String, axis: UILayoutConstraintAxis, contentView: UIView) -> LayoutHelper
    {
        let scrollView = subviews[scrollViewKey] as! UIScrollView // you should have added it
        LayoutHelper(view: scrollView).fillWithView(contentView)
    ...
    /**
     * Convencience method when you want a UIScrollView to fill the main view and scroll in the given axis.
     * The contentView is the content of the scroll view.
     */
    public func fillWithScrollView(axis: UILayoutConstraintAxis, contentView: UIView) -> LayoutHelper
    {
        fillWithView(UIScrollView()) // scrollView is added with defaultViewKey
        return configureScrollView(LayoutHelper.defaultViewKey, axis: axis, contentView:contentView)
    }
